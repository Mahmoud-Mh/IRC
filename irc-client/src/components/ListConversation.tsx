// src/components/ListConversation.tsx
    ...
    <Box sx={{ bgcolor: "#1e1e1e", height: "100%", p: 2, display: 'flex', flexDirection: 'column' }}>
      <Box sx={{ display: "flex", gap: 1, mb: 2, justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        {view === "channel" && (
          <Button
            variant="outlined"
            sx={{ mb: 2, transition: 'background-color 0.3s', '&:hover': { backgroundColor: '#3d3d3d' } }}
      ...
      <List sx={{ flexGrow: 1, overflow: 'auto' }}>
        {view === "channel"
          ? channels.map((channel) => (
              <Fade in timeout={500} key={channel._id}>
                <ListItem
                  secondaryAction={
      ...
      <Dialog open={openCreateDialog} onClose={() => setOpenCreateDialog(false)} PaperProps={{ style: { backgroundColor: '#1e1e1e' } }}>
        <Box sx={{ p: 3 }}>
          ...
          <Button
            ...
            sx={{ mt: 2, py: 1.5, transition: 'transform 0.2s', '&:hover': { transform: 'scale(1.02)' } }}
          >
            Create
          </Button>
        </Box>
