         ...
         onModuleInit() {
             this.server.on('connection', (socket) => {
              ...
              socket.on('disconnect', () => {
                ...
                socket.on('changeNickname', async (payload: { oldNickname: string; newNickname: string }) => {
                  try {
                         const user = await this.userService.updateUserNickname(payload.oldNickname, payload.newNickname);
                         if (user) {
                             this.users[socket.id] = payload.newNickname;
                             socket.emit('nicknameChanged', { success: true, newNickname: payload.newNickname });
                          } else {
                              socket.emit('error', { success: false, message: 'Failed to update nickname' });
                         }
                    } catch (error) {
                      console.error(`[changeNickname] Error:`, error);
                      socket.emit('error', { success: false, message: 'Failed to change nickname.' });
                    }
                });
